...
describe("Kanban Board v0.4", () => {
  beforeEach(() => {
    // 访问应用 - 使用环境变量或默认值
    const baseUrl = Cypress.env("BASE_URL") || "http://localhost:8082";
    cy.visit(baseUrl);
  ...
  it("should have unified modal component design with click outside to close", () => {
    // 测试添加卡片模态框
    cy.get(".btn.btn-success").contains("Add Card").click();

    // 验证模态框存在
    cy.get(".modal-overlay").should("exist");

    // 检查模态框的CSS属性
    cy.get(".modal-overlay").should("have.css", "position", "fixed");
    cy.get(".modal-overlay").should("have.css", "top", "0px");
    cy.get(".modal-overlay").should("have.css", "left", "0px");
    cy.get(".modal-overlay").should("have.css", "right", "0px");
    cy.get(".modal-overlay").should("have.css", "bottom", "0px");
    cy.get(".modal-overlay").should("have.css", "z-index", "1000");
  ...
  it("should have optimized delete card button (show on hover, gray color)", () => {
    ...
    cy.get(".btn.btn-success").first().click({ force: true });
  ...
  it("should have add card functionality as modal", () => {
    ...
    // 验证添加卡片模态框存在
    cy.get(".modal-overlay").should("exist");
    cy.get(".modal-header h3").should("have.text", "添加卡片");

    // 填写卡片信息
    cy.get("#add-title").type("New Card via Modal");
    cy.get("#add-content").type("Card content added through modal");
    cy.get("#add-status").select(CardStatus.TODO);

    // 验证输入字段的值
    cy.get("#add-title").should("have.value", "New Card via Modal");

    // 保存卡片
    cy.get(".modal-footer .btn.btn-success").click();
